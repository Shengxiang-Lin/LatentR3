...
from model import LatentModel #, NoisyLatentModel
from latent_grpo_dataset import D3Dataset, get_prefix_data
from transformers import LogitsProcessorList, TemperatureLogitsWarper
from latent_grpo_processor import CFEnhancedLogitsProcessor, get_hash
...
def main(
    ...
    model = LatentModel.from_pretrained(base_model, torch_dtype=torch.bfloat16,
                                        use_flash_attention_2=True if base_model.lower().find("llama") > -1 else False)
    ...
    val_dataset = D3Dataset(train_file=test_data_path, tokenizer=tokenizer, info_file=info_file, max_len=2560, category=category,
    ...
    def evaluate(
        ...
        #         if beam_index != -1:
        #             sequence_scores[i][j - L] = generation_output.scores[j - L][beam_index][
        #                 generation_output.sequences[i][j]].item()
    ...
    for idx, encodings in enumerate(tqdm(new_encodings)):
        output, score, seq_score = evaluate(encodings, cf_logits=None, temperature=temperature,
                                            guidance_scale=guidance_scale, length_penalty=length_penalty)
        if idx == 0:
            print(output)
            print(score)
